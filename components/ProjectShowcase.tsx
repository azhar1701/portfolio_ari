...
const ShowcaseSkeleton: React.FC = () => (
  <div className="space-y-4">
    <SkeletonLoader className="h-8 w-3/4 bg-slate-200 dark:bg-slate-700 rounded" />
    <SkeletonLoader className="h-5 w-full bg-slate-200 dark:bg-slate-700 rounded" />
    <SkeletonLoader className="h-5 w-5/6 bg-slate-200 dark:bg-slate-700 rounded" />
    <div className="aspect-video mt-4">
      <SkeletonLoader className="h-full w-full bg-slate-200 dark:bg-slate-700 rounded-lg" />
    </div>
  </div>
);
...
const Carousel: React.FC<CarouselProps> = ({ images, title, description }) => {
    ...
    return (
        <div className="w-full">
            <h4 className="font-bold text-xl text-slate-700 dark:text-slate-200 mb-2">{title}</h4>
            <div className="relative group aspect-video w-full rounded-lg overflow-hidden shadow-lg border-2 border-slate-200 dark:border-slate-700">
                <div 
            ...
            <div className="flex justify-center py-3 space-x-2">
                {images.map((_, slideIndex) => (
                    <button 
                        ...
                        className={`w-3 h-3 rounded-full transition-colors ${currentIndex === slideIndex ? 'bg-cyan-600' : 'bg-slate-300 dark:bg-slate-600 hover:bg-cyan-400'}`}
    ...
    <Section id="showcase" title="Project Showcase" iconClass="fas fa-images">
      {showcase ? (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm">
          <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">{showcase.title}</h3>
          <p className="text-slate-600 dark:text-slate-300 mb-6">{showcase.description}</p>
